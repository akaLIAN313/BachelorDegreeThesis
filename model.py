...
class HGINLayer(nn.Module):
    def __init__(self, paths, in_size, out_size, num_mlp_layers, target_node_type, device):
    ...
    def forward(self, G: dgl.heterograph, feat_dict):
        """
...
class HGIN(nn.Module):
    def __init__(self, G, features, target_node_type, paths, in_size, hidden_size, out_size, num_layers, num_mlp_layers, dropout, device):
        ...
        self.layers.append(HGINLayer(paths, in_size, hidden_size,
                           num_mlp_layers, target_node_type, device).to(self.device))
    ...
    def forward(self, G):
        ...
        for gnn in self.layers:
            h_dict = gnn(G, h_dict)
            h_dict = {ntype: F.leaky_relu(F.dropout(
                embed, self.dropout, training=self.training)) for ntype, embed in h_dict.items()}
